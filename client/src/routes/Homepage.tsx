...
const Homepage = () => {
  ...
  const [featuredBlog, setFeaturedBlog] = useState<IPost[]>([]);
    ...
    const fetchBlogs = async () => {
      try {
        ...
        const featuredBlogResponse = await axios.get(
          `${API_URL}/blogs?isFeatured=true`
        );
        const featuredBlog = featuredBlogResponse.data?.posts || [];
  ...
  return (
    <>
      <SEO
        title="Empire Report - Where Adventure Meets Awareness"
        description="Welcome to Empire Report where adventure meets awareness. Discover travel guides, breaking news, cultural stories, and visual journeys from around the world."
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex sm:flex-row flex-col gap-y-5 sm:gap-0 items-center justify-between md:mb-5 border-b border-[#2c586a]/60 pb-10">
            <div>
              ...
              <p className="mt-5 sm:mt-10 md:text-xl max-w-4xl">
          ...
          <div>
            {isLoading ? (
              ...
              <FeaturedPosts post={featuredBlog} setPost={setFeaturedBlog} />
            )}
            ...
            {isLoading ? (
              ...
              <div className="flex justify-center items-center h-[50vh]">
                <h1 className="text-2xl text-gray-600">No blog found</h1>
              </div>
            )}
          </div>
        </div>
